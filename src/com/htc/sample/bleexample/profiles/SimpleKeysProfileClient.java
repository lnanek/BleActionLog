 ...
 * 
 */
public class SimpleKeysProfileClient extends BleClientProfile implements
		ExampleActions {

	private static String TAG = SimpleKeysProfileClient.class.getSimpleName();

	private SimpleKeysServiceClient mService;

	private Context mContext;
	
	private byte mEncryption;

	public SimpleKeysProfileClient(final Context aContext, final byte aEncryption) {
		super(aContext, new BleGattID(UUID.randomUUID()));
		Log.d(TAG, "Contructor...");
		mContext = aContext;
		mEncryption = aEncryption;
		mService = new SimpleKeysServiceClient(aContext);
	...
	public void onDeviceConnected(final BluetoothDevice aDevice) {
		...
		Log.d(TAG, "Registering for button notifications...");
		mService.registerForNotification(aDevice, 0, new BleGattID(
				TiBleConstants.SIMPLE_KEYS_KEYPRESSED_UUID));
